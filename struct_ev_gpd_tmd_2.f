...
C-----------------------------------------------------------------------
C
    	subroutine struct_ev_gpd_tmd(qsquare,zeta,t,jgpd,ierr,itmd,jt)
...
*** GPDs  
        real*8 hu,r1u,hu_plus,hu_minus,
     > eu,r2u,eu_plus,eu_minus,
     > hutil,hutil_plus,hutil_minus,
     > eutil,eutil_plus,eutil_minus,
     > hd,r1d,hd_plus,hd_minus,
     > ed,r2d,ed_plus,ed_minus,
...
* Note that the distributions fill the array y()
*************************************************************************
*************************************************************************
*** INITIAL SCALE
...
*
      ...
      if(q2(i).le.xmass(1)) then 
         xlam = 0.246d0 !!! Ahmad et al, PRD 2005
      else
...
c       zeta = zeta1
      if(z.ge.zeta) then
c      call diquark_sub_pi0(z,qsquare,zeta,t,hu,hd,eu,ed)
*** For tilde part 
*** From SLAC modified parametrization
c         a1p=z**0.7
c         a1n=z**(-0.635)*(-0.013-0.33*z+0.761*z**2)
** this is obtained assuming dv=1/2 uv
c         a1u = (16.*a1p-a1n)/15. + (a1p-16.*a1n)/15./2.
*         gam2 = 4.*0.88*z**2/0.0936
*sigma g1new = f1*x* a1u 
* (1.+r)/(1.+ gam2)

c         go to (1200,1300)itmd
 1200  continue   
       call diquark_sub_GGLA(z,qsquare,zeta,t,
     > hu,r1u,hu_plus,hu_minus,
     > eu,r2u,eu_plus,eu_minus,
     > hutil,hutil_plus,hutil_minus,
     > eutil,eutil_plus,eutil_minus,
     > hd,r1d,hd_plus,hd_minus,
     ...
     > hdtil,hdtil_plus,hdtil_minus,
     > edtil,edtil_plus,edtil_minus)
c       print 600,z,qsquare,zeta,t,hu,hd,i,xbj(i)
c         go to 1600
c1300    continue
c       call diquark_BoerM_GGLA(z,qsquare,tu,td)
c      uv = z*tu(jt)
c      dv = z*td(jt)
c     
c 1600   continue
...
*** I am changing the Regge factor for the polarized case
c        sc = (hu -0.5*hd)*2./3.
c        ax = hd
c        g1u = (1.5*sc - ax/6.)*z**(0.32)
c        g1d = -hd/3.*z**(0.045)


c      u_p=z* eu  /(akappau/2.)
c      d_p=z* ed  /akappad
      if(itmd.eq.1) then
      if(ierr.eq.1) then
...
c         print 600,y(i),y(i+num),xbj(i),q2(i),q002,float(ierr)
      END DO
...
***
*** Pgq
**** SAME HERE I DON'T UNDERSTAND THE 1-ZETA/2 TERM IN A1 GOLEC BIERNAT MARTIN
        if(nlo.eq.0) then

         YPRIME(I+7*NUM)=alp0*cf/YPOINT*(1.+(1.-YY)*(1.-YYP))*Y(8*NUM+J)
     1	   *XJACOBIAN + YPRIME(I+7*NUM)       
...
***
*** Pgg
! 2*C_A= 6 ==> for GPDs the 2 cancels
! NOTICE THAT IN THE second term XPOINT/YPOINT cancels with XPOINT/YPOINT in the denominator of the non singular piece
! in Pgg
! there is an error in Golec Biernat Martin Eq. A1 the first term in Pgg should not be (x/z)^2+ (x'/z')^2 but it should
! be just (x/z) + (x'/z')
            if(nlo.eq.0) then
       	  YPRIME(I+7*NUM)=ALP0*( XPOINT/YPOINT/YPOINT*(3.*(YY+YYP)
     1  *Y(J+7*NUM) - 3.*(1.+YYP/YY)*Y(I+7*NUM) )

     1  /(1.-YY) + 3.*(1.-YYP)/YPOINT*(1.+YY*YYP)
...
***
*** Pqg
*** THERE IS A FACTOR (1-ZETA/2) IN GOLEC BIERNAT MARTIN EQ A1 THAT I DON'T UNDERSTAND, I DID NOT INCLUDE IT 
          if(nlo.eq.0) then
           YPRIME(I+8*NUM) = alp0*(XPOINT/YPOINT**2

C     2   *Y(J+7*NUM)*( .5-YY*(1.-YY) ) *2.*float(nflavor) 
     2  *Y(J+7*NUM)*(.5- YY*( (1.+ YYP/YY)/2. -YYP) ) *2.*float(nflavor) 

     2  )*XJACOBIAN + YPRIME(I+8*NUM)
...
*        aa=- y(i)*fac0*alp02 *(8./3.) /2./pi/xpoint
*     > *xjacobian 
*        bb= y(I)*cf*( alp0*(1.5 + 2.*EXTRA) )/ (2.*pi)
*        cc= y(i)*cf*
*     2  fac0*alp02*(pi*pi/3. + 0.5 - extra2)
*     1  / (2.*pi)  
...
c	 write(6,*)I,dx(i),x(i),xmax(i)
c	 write(8,*)I,dx(i),x(i),xmax(i)
	enddo
	do I=1,NUM
	   do L=1,8
c	 write(6,*)I,L,I+L*NUM
	   DX(I+L*NUM)=DX(I)
	   enddo
	   enddo
...
C
  250 CONTINUE
      ...
      IOUT=NOUT*(KSTP/NOUT)-KSTP
      IF((IOUT.NE.0) .AND. (KSTP.NE.NSTEP)) GO TO 300
      CALL FAROUT
